<template>
  <div class="cond-group" :class="{ 'is-root': isRoot }">
    <div class="rail">
      ...
      <div class="logic-switch" :class="{ 'is-or': model.logic === 'or' }">
        <button
          class="logic-btn"
          :class="{ active: model.logic === 'and' }"
          type="button"
          @click="model.logic = 'and'"
        >AND</button>
        <button
          class="logic-btn"
          :class="{ active: model.logic === 'or' }"
          type="button"
          @click="model.logic = 'or'"
        >OR</button>
      </div>
    ...
    <div class="body">
      <div class="children">
        <template v-for="(child, idx) in model.children" :key="child.id">
          <CondGroup
            v-if="child.type === 'group'"
            :model-value="child"
            :fields="fields"
            @update:model-value="val => (model.children[idx] = val)"
        ...
        <div v-if="model.children.length === 0" class="empty">
          暂无条件，点击下方按钮添加
        </div>
      </div>
...
function addCondition() {
  ...
  model.value.children.push(createCondition(props.fields[0]))
}

function addGroup() {
  // 新组默认用相反的关系，符合"组之间换种关系"的直觉
  const innerLogic = model.value.logic === 'and' ? 'or' : 'and'
